...
def build_test_app(router, places: list[Place]) -> FastAPI:
    ...
    app.state.places/known_regions/known_cities/city_to_region/
    known_place_names/place_name_to_region set directly from a fixture
    list — bypasses the real lifespan (file loading/encoding-fix/
    validation), which is tested separately in test_main.py."""
    app = FastAPI()
    ...
    app.state.known_place_names = sorted({p.name for p in places})
    app.state.place_name_to_region = {p.name: p.region for p in places}
